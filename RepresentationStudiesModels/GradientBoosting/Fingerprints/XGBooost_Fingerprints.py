...
# Function to train and evaluate XGBoost with k-Fold Cross Validation
def train_xgboost(property_name):
    ...
    # Save trained model
    joblib.dump(model, f"xgb_model_{property_name}.pkl")

    print(f"XGBoost Model for {property_name} trained and saved.")

# Train and evaluate XGBoost models for each property
...
print("Cross-validation results saved to 'xgb_cross_validation_results.csv'.")

# Perform SHAP analysis on 15% of the data
def explain_model(property_name):
    # Load trained model
    try:
        model = joblib.load(f"xgb_model_{property_name}.pkl")
    ...
    shap_df = pd.DataFrame(shap_values.values, columns=fingerprint_cols)
