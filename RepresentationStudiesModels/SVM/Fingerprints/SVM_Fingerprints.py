...
# Function to train and evaluate SVM with k-Fold Cross Validation
def train_svm(property_name):
    ...
    # Save trained model
    joblib.dump(model, f"svm_model_{property_name}.pkl")

    print(f"SVM Model for {property_name} trained and saved.")

# Train and evaluate SVM models for each property
for prop in properties:
    train_svm(prop)

# Save cross-validation results to CSV
cv_results_df = pd.DataFrame(cv_results)
cv_results_df.to_csv("svm_cross_validation_results.csv", index=False)

print("Cross-validation results saved to 'svm_cross_validation_results.csv'.")

# Perform SHAP analysis on 15% of the data
def explain_model(property_name):
    # Load trained model
    try:
        model = joblib.load(f"svm_model_{property_name}.pkl")
